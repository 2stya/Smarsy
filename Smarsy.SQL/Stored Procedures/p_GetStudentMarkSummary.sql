...
BEGIN
    ...
    WHERE s.Id = @studentId
          AND
          (
              CAST(DATEADD(DAY, @daysToShow * -1, SYSDATETIME()) AS DATE) <= CAST(sm.CreateDtime AS DATE)
              OR CAST(DATEADD(DAY, @daysToShow * -1, SYSDATETIME()) AS DATE) <= CAST(sm.MarkDate AS DATE)
          )
